...
class ScreenStore extends Accessor {
  ...
  protected async goToCamera(camera: Camera, view: SceneView) {
    this._cameraAnimationAbortController?.abort();
    const { signal } = (this._cameraAnimationAbortController = this.createAbortController());
    return ignoreAbortErrors(view.goTo(camera, { animate: true, speedFactor: transitionCameraAnimationSpeedFactor, signal }));
  }
