...
const vectorFromTwoPoints = (from: Point, to: Point) => new Point({
...
const magnitude = (point: Point) => Math.sqrt(
...
const createSagLine = (from: Point, to: Point, sagToSpanRatio: number) => {
  ...
  console.log({ count, spanLength2D, fromPointZ, toPointZ, listT, listZ });
