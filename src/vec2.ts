...
export function scale(vec: number[], factor: number): number[] {
...
export function distance(vecA: number[], vecB: number[]): number {
  const dx = vecB[0] - vecA[0];
  const dy = vecB[1] - vecA[1];
  return Math.sqrt(dx ** 2 + dy ** 2);
}

export function dot(vecA: number[], vecB: number[]): number {
  return vecA[0] * vecB[0] + vecA[1] * vecB[1];
}

export function equals(vecA: number[], vecB: number[]): boolean {
  return vecA[0] === vecB[0] && vecA[1] === vecB[1];
}
